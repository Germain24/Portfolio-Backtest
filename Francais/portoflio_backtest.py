...
def get_portfolio_value(date_debut, date_fin):
    ...
    df0 = pd.DataFrame(index=pd.date_range(start=date_debut, end=date_fin, freq='D'), columns=['changement']).fillna(value=0)
...
print(f"- Ratio de Sharpe : {round(float(valeur_portefeuille.pct_change().mean() / valeur_portefeuille.pct_change().std() * 365.25 ** 0.5),2)}.")
print(f"- Maximum Drawdown : {round(float((( valeur_portefeuille / valeur_portefeuille.cummax()) - 1).min())*100,2)}%.")
